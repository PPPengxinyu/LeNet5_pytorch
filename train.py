...
# 加载训练集
data_train_loader = DataLoader(trainDataset, batch_size=256, shuffle=True, num_workers=8)

# 实例化模型
model = LeNet5()
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") # 判断设备
print(device)
model = model.to(device) #将模型搬移到GPU上
model.train() # 切换到训练模式
...
for epoch in range(epochs):
    ...
    #打印结果
    print('epoch:{}'.format(epoch), '------ loss:%.3f | acc:%.3f%%(%d/%d)'%(train_loss/(batch_idx+1), 100.*correct/total, correct, total))
