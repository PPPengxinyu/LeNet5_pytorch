...
# s实例化测试数据集
# 实例化数据集类型
testDataset = MyDataset('data/MNIST/raw', 't10k-images-idx3-ubyte.gz', 't10k-labels-idx1-ubyte.gz', transform=transforms.ToTensor())
data_test_loader = DataLoader(testDataset, batch_size=256, shuffle=True, num_workers=8)

# 加载模型
model = torch.load('./weight/LeNet.pkl')
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") # 检查gpu可用性
